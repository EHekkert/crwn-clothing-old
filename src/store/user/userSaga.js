...
export function* signInWithTauw() {
    try {
        const { user } = yield call(signInWithMicrosoftPopup);
        yield call(getSnapshotFromUserAuth, user);
    } catch(error) {
        yield put(signInFailed(error));
    }
...
export function* onTauwSignInStart() {
    yield takeLatest(USER_ACTION_TYPES.TAUW_SIGN_IN_START, signInWithTauw);
}
